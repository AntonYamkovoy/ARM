...
modwhile 
		...
		BLO exitModWhile   ; if remainder < b exit the loop        stores the remainder in R9  (modulus operator action can be transfered here and given the remainder as output) 
...
invalidinput
		...
		BL sendchar
		MOV R0, #0x61   
		BL sendchar              ; printing out the message
		MOV R0, #0x6C   
		BL sendchar
		MOV R0, #0x69           ; "invalid input!"
		BL sendchar
		MOV R0, #0x64   
		BL sendchar
		MOV R0, #0x20   
